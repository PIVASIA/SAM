...
'''
grounding_caption = "signboard store"
box_threshold, text_threshold, box_size_threshold, reset_image = 0.35, 0.35, 0.5, True
...
segtracker_args = {
    'sam_gap': 2, # the interval to run sam to segment new objects
    'min_area': 400, # minimal mask area to add a new mask as a new object
    'max_obj_num': 255, # maximal object number to track in a video
    'min_new_obj_iou': 0.3, # the area of a new object in the background should > 80% 
}
...
with torch.cuda.amp.autocast():
    while cap.isOpened():
        ...
        if frame_idx == 0:
            ...
            # seg_mask1 = np.zeros((height, width))
        ...
        save_prediction(pred_mask,output_dir,"out{:0>3}".format(str(frame_idx+1))+'.png')
        # save_prediction(masked_frame,"./assets/demo_seg","seg{:0>3}".format(str(frame_idx+1))+'.png')
        # masked_frame = draw_mask(frame,pred_mask)
